    ...
    */
    protected function _loadFormFields(Zend_Pdf_Element_Reference $root)
    {
      ...
      foreach ($root->AcroForm->Fields->items as $field)
      {
          ...
          if ( $field->FT->value == 'Btn' && $field->T !== null ) /* We only support fields that are textfields and have a name */
          {
              $this->_formFields[$field->T->value] = $field;
          }
    ...
    public function setRadioField($name, $value)
    {
      if ( !isset($this->_formFields[$name]))
        throw new Zend_Pdf_Exception("Field '$name' does not exist or is not a textfield");
      
      $field = $this->_formFields[$name];
      $field->__set('V',new Zend_Pdf_Element_Radio($value));
      $field->touch();      
    }


    public function setCheckField($name, $value)
    {
      if ( !isset($this->_formFields[$name]))
        throw new Zend_Pdf_Exception("Field '$name' does not exist or is not a textfield");
      
      $field = $this->_formFields[$name];
      $field->__set('AS',new Zend_Pdf_Element_Check(1));
      $field->touch();      
    }
